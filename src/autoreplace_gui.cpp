...
class ReplaceVehicleWindow : public Window {
	...
	void ReplaceClick_StartReplace(bool replace_when_old)
	{
		...
		DoCommandP(0, (replace_when_old ? 1 : 0) | (this->sel_group << 16), veh_from + (veh_to << 16), 0, CMD_SET_AUTOREPLACE);
	}
	...
	void OnClick(Point pt, int widget, int click_count) override
	{
		switch (widget) {
			...
			case WID_RV_TRAIN_WAGONREMOVE_TOGGLE: {
				const Group *g = Group::GetIfValid(this->sel_group);
				if (g != nullptr) {
					DoCommandP(0, this->sel_group | (GroupFlags::GF_REPLACE_WAGON_REMOVAL << 16), (HasBit(g->flags, GroupFlags::GF_REPLACE_WAGON_REMOVAL) ? 0 : 1) | (_ctrl_pressed << 1), 0, CMD_SET_GROUP_FLAG);
				} else {
					// toggle renew_keep_length
					DoCommandP(0, GetCompanySettingIndex("company.renew_keep_length"), Company::Get(_local_company)->settings.renew_keep_length ? 0 : 1, 0, CMD_CHANGE_COMPANY_SETTING);
				}
				break;
			}
			...
			case WID_RV_STOP_REPLACE: { // Stop replacing
				EngineID veh_from = this->sel_engine[0];
				DoCommandP(0, this->sel_group << 16, veh_from + (INVALID_ENGINE << 16), 0, CMD_SET_AUTOREPLACE);
				break;
			}
