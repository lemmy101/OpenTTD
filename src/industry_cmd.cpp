...
static bool SearchLumberMillTrees(TileIndex tile, void *user_data)
{
	if (IsTileType(tile, MP_TREES) && GetTreeGrowth(tile) > 2) { ///< 3 and up means all fully grown trees
		...
		DoCommand(tile, 0, 0, 0, DC_EXEC, CMD_LANDSCAPE_CLEAR);
	...
	for (const IndustryTileLayoutTile &it : layout) {
		...
		if (gfx == GFX_WATERTILE_SPECIALCHECK) {
			...
			if ((ind_behav & (INDUSTRYBEH_ONLY_INTOWN | INDUSTRYBEH_TOWN1200_MORE)) || // Tile must be a house
				...
				CommandCost ret = DoCommand(cur_tile, 0, 0, 0, DC_NONE, CMD_LANDSCAPE_CLEAR);
				cur_company.Restore();

				if (ret.Failed()) return ret;
			} else {
				/* Clear the tiles, but do not affect town ratings */
				CommandCost ret = DoCommand(cur_tile, 0, 0, 0, DC_AUTO | DC_NO_TEST_TOWN_RATING | DC_NO_MODIFY_TOWN_RATING, CMD_LANDSCAPE_CLEAR);
...
static bool CheckIfCanLevelIndustryPlatform(TileIndex tile, DoCommandFlag flags, const IndustryTileLayout &layout, int type)
{
	...
	TILE_AREA_LOOP(tile_walk, ta) {
		...
		if (curh != h) {
			...
			if (DoCommand(tile_walk, SLOPE_N, (curh > h) ? 0 : 1, 0, flags & ~DC_EXEC, CMD_TERRAFORM_LAND).Failed()) {
				cur_company.Restore();
				return false;
			}
		}
	...
	if (flags & DC_EXEC) {
		...
		TILE_AREA_LOOP(tile_walk, ta) {
			...
			while (curh != h) {
				...
				DoCommand(tile_walk, SLOPE_N, (curh > h) ? 0 : 1, 0, flags, CMD_TERRAFORM_LAND);
				curh += (curh > h) ? -1 : 1;
			}
		}
	}
...
static void DoCreateNewIndustry(Industry *i, TileIndex tile, IndustryType type, const IndustryTileLayout &layout, size_t layout_index, Town *t, Owner founder, uint16 initial_random_bits)
{
	...
	for (const IndustryTileLayoutTile &it : layout) {
		...
		if (it.gfx != GFX_WATERTILE_SPECIALCHECK) {
			...
			DoCommand(cur_tile, 0, 0, 0, DC_EXEC | DC_NO_TEST_TOWN_RATING | DC_NO_MODIFY_TOWN_RATING, CMD_LANDSCAPE_CLEAR);
 ...
 */
CommandCost CmdBuildIndustry(TileIndex tile, DoCommandFlag flags, uint32 p1, uint32 p2, uint64 p3, const char *text)
{
 ...
 */
CommandCost CmdIndustryCtrl(TileIndex tile, DoCommandFlag flags, uint32 p1, uint32 p2, uint64 p3, const char *text)
{
