 ...
 */
CommandCost CmdBuildSingleRail(TileIndex tile, DoCommandFlag flags, uint32 p1, uint32 p2, uint64 p3, const char *text)
{
	...
	switch (GetTileType(tile)) {
		case MP_RAILWAY: {
			...
			if (!IsPlainRail(tile)) return DoCommand(tile, 0, 0, 0, flags, CMD_LANDSCAPE_CLEAR); // just get appropriate error message
			...
			if (HasSignals(tile) && TracksOverlap(GetTrackBits(tile) | TrackToTrackBits(track))) {
				...
				for (Track track_it = TRACK_BEGIN; track_it < TRACK_END; track_it++) {
					if (HasTrack(tile, track_it) && HasSignalOnTrack(tile, track_it)) {
						CommandCost ret_remove_signals = DoCommand(tile, track_it, 0, 0, flags, CMD_REMOVE_SIGNALS);
			...
			if (GetRailType(tile) != railtype && !HasPowerOnRail(railtype, GetRailType(tile))) {
				if (HasPowerOnRail(GetRailType(tile), railtype)) {
					ret = DoCommand(tile, tile, railtype, 0, flags, CMD_CONVERT_RAIL);
					if (ret.Failed()) return ret;
					cost.AddCost(ret);
				} else {
					return CMD_ERROR;
		...
		default: {
			...
			ret = DoCommand(tile, 0, 0, 0, flags, CMD_LANDSCAPE_CLEAR);
			if (ret.Failed()) return ret;
			cost.AddCost(ret);
 ...
 * @return the cost of this operation or an error
 */
CommandCost CmdRemoveSingleRail(TileIndex tile, DoCommandFlag flags, uint32 p1, uint32 p2, uint64 p3, const char *text)
{
	Track track = Extract<Track, 0, 3>(p2);
	CommandCost cost(EXPENSES_CONSTRUCTION);
	bool crossing = false;
	...
	switch (GetTileType(tile)) {
		...
		case MP_RAILWAY: {
			...
			if (HasSignalOnTrack(tile, track)) {
				cost.AddCost(DoCommand(tile, track, 0, 0, flags, CMD_REMOVE_SIGNALS));
			}
...
bool FloodHalftile(TileIndex t)
{
	...
	if (IsSlopeWithOneCornerRaised(tileh)) {
		...
		if (to_remove != 0) {
			...
			flooded = DoCommand(t, 0, FIND_FIRST_BIT(to_remove), 0, DC_EXEC, CMD_REMOVE_SINGLE_RAIL).Succeeded();
			cur_company.Restore();
 ...
 * @return the cost of this operation or an error
 */
static CommandCost CmdRailTrackHelper(TileIndex tile, DoCommandFlag flags, uint32 p1, uint32 p2, uint64 p3, const char *text)
{
	...
	for (;;) {
		CommandCost ret = DoCommand(tile, remove ? 0 : railtype, TrackdirToTrack(trackdir) | (auto_remove_signals << 3), 0, flags, remove ? CMD_REMOVE_SINGLE_RAIL : CMD_BUILD_SINGLE_RAIL);
 ...
 */
CommandCost CmdBuildRailroadTrack(TileIndex tile, DoCommandFlag flags, uint32 p1, uint32 p2, uint64 p3, const char *text)
{
	return CmdRailTrackHelper(tile, flags, p1, ClrBit(p2, 9), p3, text);
}
 ...
 * @see CmdRailTrackHelper
 */
CommandCost CmdRemoveRailroadTrack(TileIndex tile, DoCommandFlag flags, uint32 p1, uint32 p2, uint64 p3, const char *text)
{
	return CmdRailTrackHelper(tile, flags, p1, SetBit(p2, 9), p3, text);
}
 ...
 */
CommandCost CmdBuildTrainDepot(TileIndex tile, DoCommandFlag flags, uint32 p1, uint32 p2, uint64 p3, const char *text)
{
	...
	cost.AddCost(DoCommand(tile, 0, 0, 0, flags, CMD_LANDSCAPE_CLEAR));
 ...
 */
CommandCost CmdBuildSingleSignal(TileIndex tile, DoCommandFlag flags, uint32 p1, uint32 p2, uint64 p3, const char *text)
{
...
static CommandCost CmdSignalTrackHelper(TileIndex tile, DoCommandFlag flags, uint32 p1, uint32 p2, uint64 p3, const char *text)
{
	...
	for (;;) {
		...
		if (remove || minimise_gaps || signal_ctr % signal_density == 0) {
			...
			CommandCost ret = DoCommand(tile, p1, signals, 0, test_only ? flags & ~DC_EXEC : flags, remove ? CMD_REMOVE_SIGNALS : CMD_BUILD_SIGNALS);
			...
			if (ret.Succeeded()) {
				...
				ret = DoCommand(last_suitable_tile, p1, signals, 0, flags, remove ? CMD_REMOVE_SIGNALS : CMD_BUILD_SIGNALS);
			}
 ...
 */
CommandCost CmdBuildSignalTrack(TileIndex tile, DoCommandFlag flags, uint32 p1, uint32 p2, uint64 p3, const char *text)
{
 ...
 * @return the cost of this operation or an error
 */
CommandCost CmdRemoveSingleSignal(TileIndex tile, DoCommandFlag flags, uint32 p1, uint32 p2, uint64 p3, const char *text)
{
	Track track = Extract<Track, 0, 3>(p1);
 ...
 * @see CmdSignalTrackHelper
 */
CommandCost CmdRemoveSignalTrack(TileIndex tile, DoCommandFlag flags, uint32 p1, uint32 p2, uint64 p3, const char *text)
{
 ...
 * @return the cost of this operation or an error
 */
CommandCost CmdConvertRail(TileIndex tile, DoCommandFlag flags, uint32 p1, uint32 p2, uint64 p3, const char *text)
{
	...
	switch (GetRailTileType(tile)) {
		...
		case RAIL_TILE_NORMAL: {
			...
			while (tracks != TRACK_BIT_NONE) {
				...
				CommandCost ret = DoCommand(tile, 0, track, 0, flags, CMD_REMOVE_SINGLE_RAIL);
				if (ret.Failed()) return ret;
				cost.AddCost(ret);
			}
...
static void ChangeTileOwner_Track(TileIndex tile, Owner old_owner, Owner new_owner)
{
	...
	if (new_owner != INVALID_OWNER) {
		...
		DoCommand(tile, 0, 0, 0, DC_EXEC | DC_BANKRUPT, CMD_LANDSCAPE_CLEAR);
	}
}
