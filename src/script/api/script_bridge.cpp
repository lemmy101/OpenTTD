...
/* static */ bool ScriptBridge::BuildBridge(ScriptVehicle::VehicleType vehicle_type, BridgeID bridge_id, TileIndex start, TileIndex end)
{
	...
	if (vehicle_type == ScriptVehicle::VT_RAIL || vehicle_type == ScriptVehicle::VT_WATER) {
		return ScriptObject::DoCommand(end, start, type | bridge_id, 0, CMD_BUILD_BRIDGE);
	}

	ScriptObject::SetCallbackVariable(0, start);
	ScriptObject::SetCallbackVariable(1, end);
	return ScriptObject::DoCommand(end, start, type | bridge_id, 0, CMD_BUILD_BRIDGE, nullptr, &::_DoCommandReturnBuildBridge1);
}
...
/* static */ bool ScriptBridge::_BuildBridgeRoad1()
{
	...
	return ScriptObject::DoCommand(start + ::TileOffsByDiagDir(dir_1), ::DiagDirToRoadBits(dir_2) | (ScriptRoad::GetCurrentRoadType() << 4), 0, 0, CMD_BUILD_ROAD, nullptr, &::_DoCommandReturnBuildBridge2);
}
...
/* static */ bool ScriptBridge::_BuildBridgeRoad2()
{
	...
	return ScriptObject::DoCommand(end + ::TileOffsByDiagDir(dir_2), ::DiagDirToRoadBits(dir_1) | (ScriptRoad::GetCurrentRoadType() << 4), 0, 0, CMD_BUILD_ROAD);
}
