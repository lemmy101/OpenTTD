...
struct RefitWindow : public Window {
	...
	StringID GetCapacityString(RefitOption *option) const
	{
		...
		CommandCost cost = DoCommand(v->tile, this->selected_vehicle, option->cargo | option->subtype << 8 | this->num_vehicles << 16 |
				(int)this->auto_refit << 24, 0, DC_QUERY_COST, GetCmdRefitVeh(v->type));
			...
			case WID_VR_REFIT: // refit button
				if (this->cargo != nullptr) {
					...
					if (this->order == INVALID_VEH_ORDER_ID) {
						...
						if (DoCommandP(v->tile, this->selected_vehicle, this->cargo->cargo | this->cargo->subtype << 8 | this->num_vehicles << 16, 0, GetCmdRefitVeh(v)) && delete_window) delete this;
					} else {
						if (DoCommandP(v->tile, v->index, this->cargo->cargo | this->order << 16, 0, CMD_ORDER_REFIT)) delete this;
					}
				}
				break;
		}
	...
	void OnClick(Point pt, int widget, int click_count) override
	{
		switch (widget) {
			...
			case WID_VL_START_ALL:
				DoCommandP(0, (1 << 1) | (widget == WID_VL_START_ALL ? (1 << 0) : 0), this->window_number, 0, CMD_MASS_START_STOP);
				break;
		}
	}
			...
			case WID_VL_MANAGE_VEHICLES_DROPDOWN:
				...
				switch (index) {
					...
					case ADI_DEPOT: // Send to Depots
						DoCommandP(0, DEPOT_MASS_SEND | (index == ADI_SERVICE ? DEPOT_SERVICE : (DepotCommand)0), this->window_number, 0, GetCmdSendToDepot(this->vli.vtype));
						break;
	...
	void OnClick(Point pt, int widget, int click_count) override
	{
		switch (widget) {
			...
			case WID_VD_DECREASE_SERVICING_INTERVAL: { // decrease int
				...
				DoCommandP(v->tile, v->index, mod | (1 << 16) | (v->ServiceIntervalIsPercent() << 17), 0, CMD_CHANGE_SERVICE_INT | CMD_MSG(STR_ERROR_CAN_T_CHANGE_SERVICING));
				break;
			}
	...
	void OnDropdownSelect(int widget, int index) override
	{
		switch (widget) {
			case WID_VD_SERVICE_INTERVAL_DROPDOWN: {
				...
				DoCommandP(v->tile, v->index, interval | (iscustom << 16) | (ispercent << 17), 0, CMD_CHANGE_SERVICE_INT | CMD_MSG(STR_ERROR_CAN_T_CHANGE_SERVICING));
				break;
			}
		}
	}
...
void StartStopVehicle(const Vehicle *v, bool texteffect)
{
	...
	DoCommandP(v->tile, v->index, 0, 0, _vehicle_command_translation_table[VCT_CMD_START_STOP][v->type], texteffect ? CcStartStopVehicle : nullptr);
}
...
public:
	...
	void OnClick(Point pt, int widget, int click_count) override
	{
		...
		switch (widget) {
			...
			case WID_VV_GOTO_DEPOT: // goto hangar
				DoCommandP(v->tile, v->index | (_ctrl_pressed ? DEPOT_SERVICE : 0U), 0, 0, GetCmdSendToDepot(v));
				break;
	...
	void OnQueryTextFinished(char *str) override
	{
		...
		DoCommandP(0, this->window_number, 0, 0, CMD_RENAME_VEHICLE | CMD_MSG(STR_ERROR_CAN_T_RENAME_TRAIN + Vehicle::Get(this->window_number)->type), nullptr, str);
	}
